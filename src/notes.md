open in src folder 

MUST RUN:
pip install fpdf2
in terminal before using application 

use the following command to run the program:
python RecipeGUI.py


bella
-made homepage
-create classes
-made it add recipes && store them
-make button to new page work
-frontend logic for second page



anna
- backend logic for second page, including search logic and generating a grocery list
- added preexisting recipes to recipe book
- made all recipes data the homepage, with an add recipes button to the original homepage
- made add to grocery list button, clear grocery list button, and generate list button

need to do:
- display selected recipe info in the main homepage (DONE)
- make the grocery list buttons functional (DONE)
- make pretty!!!